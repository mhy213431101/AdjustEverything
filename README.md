...
功能1 误差方程计算器 MatrixCalculator
- 四种平差建模方法的误差方程计算

功能2 平差画板 DrawingBoard
- 在画板上添加、拖动点
- 添加普通连线
- 添加高差观测
- 添加距离观测
...
- 设置已知高程点
- 设置已知平面坐标点
- 画布采用测量学坐标约定：向上为 X 正方向，向右为 Y 正方向
- 角度观测支持 `dd°mm'ss"` 格式输入与显示
- 编辑点名、X/Y、高程 H、观测值和中误差
- 删除点、观测和关联连线
- 载入水准/测边综合示例网、测角示例网、边角示例网
- 导入、导出项目文件（`.aep.json`）
- 导出平差结果文本
- 水准网计算前检查
- 测边网计算前检查
...
AngleDistanceModel.cs
AngleFormatter.cs
AngleModel.cs
ApproximateCoordinateBuilder.cs
ApproximateHeightBuilder.cs
DistanceModel.cs
DrawingBoard.cs
FormHome.cs
FormDrawingBoard.cs
...
程序入口文件。它调用 WinForms 的初始化逻辑，并启动主页窗体 `FormHome`。

### `FormHome.cs`

主页窗口

1. 负责导航至`FormDrawingBoard`和`MatrixCalculator`两个功能窗口
2. “关于我们” - 显示制作信息
...
- 响应示例网载入、项目导入、项目导出、结果导出按钮
- 接收画板发出的项目变更和对象选中事件
- 根据选中对象动态生成属性编辑面板
- 调用 `ProjectDiagnostics` 做计算前检查
- 构建输入数据n, t, X0
- 调用 `Approximate...Builder.cs`计算未知参数近似值
- 调用 `...Model` 进行平差建模
- 调用`NonlinearLeastSquaresSolver` 进行平差解算
- 调用`PrecisionEstimator` 进行精度评定
- 组合生成平差结果报告
- 把平差结果写回点对象，并刷新界面
- 使用 `AngleFormatter` 处理角度观测的度分秒输入与显示
- 使用 `ProjectFileService` 保存和读取项目文件
...
左侧参数输入面板、中间误差方程矩阵编辑区、右侧结果显示面板、上侧平差模型切换

主要负责：
- 响应功能按钮，例如创建设计矩阵、生成权阵、解算参数、计算残差、导出方程、清空数据
- 接收矩阵编辑区的数据变更事件，实时更新方程预览
...
- `AdjustmentProject`：项目总容器，保存点、线、已知点、已知边、高差观测、距离观测和角度观测
...
- `AngleObservation`：角度观测，表示后视点、测站点、前视点构成的水平角
...
- 绘制网格、点、线、高差标签、距离标签和角度标签
- 单击空白处添加点
- 拖动点改变画板位置
- 两次点选创建普通连线、高差观测或距离观测
- 三次点选创建角度观测（后视点、测站点、前视点）
- 单击点或观测线进行选中
- 用颜色区分普通点、已知高程点和已知坐标点

它不直接进行平差计算，只通过事件通知 `FormDrawingBoard` 刷新界面。

### `SurveyCoordinateMapper.cs`

画布坐标与测量学平面坐标之间的转换工具。

WinForms 画布坐标默认是向右为屏幕 X 正方向、向下为屏幕 Y 正方向。平差模型中使用测量学坐标约定：

- 向上为 X 正方向
- 向右为 Y 正方向

因此新增点、拖动点、测边网和边角网初值都会通过该工具统一换算，避免界面显示和模型坐标方向不一致。

### `AngleFormatter.cs`

角度格式化与解析工具。

当前角度观测内部仍以十进制度保存，进入模型时再转换为弧度；界面层显示和输入采用 `dd°mm'ss"` 格式。例如：

```text
35°05'44.94"
```

为了便于调试和录入，输入框也兼容十进制度。
...
- 网型类型（根据当前观测自动判断）
- 点的画布位置、测量坐标、高程、已知点标记
- 线、已知点、已知边、基线
- 高差观测、距离观测、角度观测
- 当前结果面板中的平差报告文本

导入时会重建 `AdjustmentProject` 中的点、线、观测关系，并刷新画板和对象列表。
...
3. 根据高差观测构造线性误差方程：
   ...
   观测向量 L
   权阵 P
4. 将模型封装为 LevelHeightModel，通过 IAdjustmentModel 与 ILinearizable 接口实现后续统一调度求解：

由于水准网本质为线性模型，因此在当前框架下：
不需要迭代过程
...
根据已知点高程，高差观测值及水准观测路线，推算未知点高程近似值X0
...
收集所有可用观测圆，在任意两圆组合中生成所有候选解
由于解不唯一，且难以先验判定，故输出所有可能解集, 在Form中调用解集穷举排列求解，根据最小VPV^T值选择几何最优解集X0

### `AngleModel.cs`

测角网间接平差建模。

角度观测按“后视点 - 测站点 - 前视点”定义。模型根据各点当前 X/Y 计算理论角值，并将观测角与理论角差值归化到合理范围后参与最小二乘迭代。角度残差统一以秒（″）为单位进入误差方程。

### `AngleDistanceModel.cs`

边角网间接平差建模。

该模型把距离观测和角度观测放入同一个非线性最小二乘框架中：

- 距离观测残差以毫米（mm）为单位
- 角度观测残差以秒（″）为单位
- 参数仍为未知点的 X/Y 坐标

当前边角网要求至少两个已知平面坐标点作为基准。
...
用户操作画板
  ...
  -> FormDrawingBoard 刷新对象列表和属性面板
  -> 用户编辑属性
  -> FormDrawingBoard 写回 AdjustmentProject
  -> 用户点击检查或计算
  -> ProjectDiagnostics 检查模型
  -> FormDrawingBoard 构造输入数据
...
- `DrawingBoard` 是图形交互层
- `FormDrawingBoard` 是界面协调层
- `SurveyCoordinateMapper` 是画布坐标和测量坐标的转换层
- `AngleFormatter` 是角度输入输出格式转换层
- `ProjectFileService` 是项目文件持久化层
- `ProjectDiagnostics` 是模型检查层
- `IAdjustmentModel` 是输入数据接口
- `ILinearizable` 是线性化接口
- `LevelHeightModel`、`DistanceModel`、`AngleModel` 和 `AngleDistanceModel` 是网型建模层
